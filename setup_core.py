...
IDENTITY_MD = """# Identity Strategy (Helix Engine)
...
*Tradeoff: These guidelines bias toward caution and surgical precision over unguided execution speed.*
...
PROFILE_MD = """# Profile Preferences (Helix Engine)

## Async-First Paradigm
All Python additions, script updates, or architecture refactors must perfectly match our FastAPI structure using modern async/await syntax, `httpx`, and asynchronous subprocess execution (`asyncio.create_subprocess_exec`).

## Strict Typing & Validation
Every single function signature must include explicit Python type hints and leverage Pydantic models for clean incoming data parsing and validation.

## Modern Web Tech Stack
Force clean semantic HTML5, utility-first or modular layout-driven CSS3, and modern ECMAScript (ES6+) web code. Do not introduce massive framework boilerplate unless requested.
"""

WORKSPACE_MD = """# Workspace Guidelines
This is the Helix Engine active workspace. All generated output must conform to IDENTITY.md and PROFILE.md.
"""
...
def setup():
    core_dir = Path("core_memory")
    core_dir.mkdir(parents=True, exist_ok=True)
    
    with open(core_dir / "IDENTITY.md", "w", encoding="utf-8") as f:
        f.write(IDENTITY_MD)
        
    with open(core_dir / "PROFILE.md", "w", encoding="utf-8") as f:
        f.write(PROFILE_MD)
        
    with open(core_dir / "WORKSPACE.md", "w", encoding="utf-8") as f:
        f.write(WORKSPACE_MD)
        
    with open(core_dir / "pipeline.json", "w", encoding="utf-8") as f:
        json.dump(PIPELINE_JSON, f, indent=4)
        
    print("✅ Helix Engine core_memory/ successfully seeded.")
